...
def run(command, capture_output=False, **kwargs):
    if isinstance(command, str):
        print(f"> {command}")
        if capture_output:
            return subprocess.check_output(command, shell=True, text=True, **kwargs)
        else:
...
def main():
    ...
    run([py, "-m", "pip", "install", "--upgrade", "pip", "setuptools", "wheel", "ipykernel"])

    if REQ_FILE.exists():
        run([py, "-m", "pip", "install", "--no-deps", "-r", str(REQ_FILE)])
    ...
    run([py, "-m", "ipykernel", "install", "--sys-prefix", "--name", "earth-miner", "--display-name", "Earth Miner Env"])
    ...
    site_packages = run([py, "-c", "import site; print(site.getsitepackages()[0])"], capture_output=True).strip()
